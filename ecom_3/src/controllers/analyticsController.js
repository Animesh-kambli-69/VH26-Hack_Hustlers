...
module.exports=asyncHandler(async(req,res)=>{
  const key=`product-analytics:${req.params.code}`; const hit=await getCached(key);
  if(hit) return res.json({...hit,cache:{hit:true}});
  const result=await get(req.params.code); if(!result) return res.status(404).json({error:"Product not found"});
